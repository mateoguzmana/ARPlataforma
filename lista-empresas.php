...
<script> 
function retornavalores(registro, nombre, id){ 
   window.opener.document.form1.Nombre.value=registro;
   window.opener.document.form1.Nit.value=nombre;
    window.opener.document.form1.Idempresa.value=id;
   window.close(); 
} 
</script>
    ...
    <tr>
      <td height="100" align="right" valign="top"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td width="94%"><span class="Estilo93">Nit</span><span class="listado2"><br>
            <input name="Nit" type="text" class="TextField" id="Nit" size="15">
...
if ($Nit <> '')
{
$paging->agregarConsulta("SELECT* FROM Empresas where Nit = '$Nit' order by Nombre");
}
elseif ($Empresa <> '' )
{
$paging->agregarConsulta("SELECT* FROM Empresas where Nombre like '%$Empresa%' order by Nombre");
}
else
{
$paging->agregarConsulta("SELECT* FROM Empresas order by Nombre");
}
...
?>
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FBFBFB" onClick="retornavalores('<?=$Nombre8?>', '<?=$Cedula8?>','<?=$Id8?>')">
 
        
          <tr name="datos1" id="datos1" onMouseover="this.bgColor='#BED9F3'"onMouseout="this.bgColor='#FBFBFB'" >
            <td width="10%" align="center"><img src="Images/botonto.jpg" width="18" height="12"></td>
            <td width="19%" class="listado"><?=$Cedula8?></td>
            <td width="71%" class="listado"><?=$Nombre8?></td>
          </tr>
