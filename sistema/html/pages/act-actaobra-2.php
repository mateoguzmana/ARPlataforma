...
$_SESSION['anterior']	=	$_SERVER['REQUEST_URI']; 
					...
					//Empezamos con la consulta de los datos a actualizar

					$query 				="SELECT * FROM Actasobra WHERE Id='$Id'";
					$result 			=mysql_query($query,$id);
					while 				($row=mysql_fetch_array($result)) {
					$ProveedorTT 		=$row["Idproveedor"];	
					$ObraTT				=$row["Idobra"];
					...
					//Fin consulta 
					$Prov 					=$_REQUEST["IdProve"];
					if(empty($Prov)){
					$Proveedor 				=$ProveedorTT;
					}
					else{
					$Proveedor 				=$Prov;
					}
...
?>
														...
														{
														?>
												<form class="form-horizontal form-validate" role="form" method="post" action="act-actaobra-3.php" id="form1" name="form1" novalidate>
                                                        <?
														}
														else
														{
                                                ...
                                                <div class="col-sm-6">
															<div class="form-group">
                                                                ...
                                                                <select name="jumpMenu" id="jumpMenu" class="form-control" onChange="MM_jumpMenu('parent',this,0)"required  data-placeholder="Seleccione un proveedor" style="background-color:#CCC">
																		...
																		{
																		?>
                                                                  <option value="act-actaobra-2.php?Opcion=<?=$TipoForm?>&IdProve=<?=$Idproveedor?>&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>" selected><?=$NombreProve?></option>
                                                                        <?
																		}
																		else
																		{
																		?>
                                                                        <option value="act-actaobra-2.php" selected>Seleccione un proveedor</option>
                                                                        <?
																		}
																		if ($TipoForm=="1") {
                                                                        ...
                                                                        ?>
                                                                        <option value="act-actaobra-2.php?Opcion=<?=$TipoForm?>&IdProve=<?=$Idproveedor?>&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>"><?=$Nombreproveedor?></option>
																		<?
																		}
																		}
																		}else{
													...
													<div class="col-sm-6">
															<div class="form-group">
                                                                ...
                                                                <select name="jumpMenu" id="jumpMenu" class="form-control" onChange="MM_jumpMenu('parent',this,0)"required  data-placeholder="Seleccione una Obra" style="background-color:#CCC">
																		...
																		{
																		?>
                                                                  <option value="act-actaobra-2.php?Idobra=<?=$Idobra?>&IdProve=<?=$Idproveedor?>&Opcion=<?=$TipoForm?>&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>" selected><?=$Nombreobra?></option>
                                                                        <?
																		}
																		else
																		{
																		?>
                                                                        <option value="act-actaobra-2.php" selected>Seleccione una Obra</option>
                                                                        <?
																		}
                                                                        ...
                                                                        ?>
                                                                        <option value="act-actaobra-2.php?Idobra=<?=$IdTIP?>&IdProve=<?=$Proveedor?>&Opcion=<?=$TipoForm?>&Idmenu=<?=$Idmenux?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>"><?=$NombreTIP?></option>
																		<?
																		}
																		}
																		?>
...
?>
														...
														<div class='input-group date' id='demo-date-start'>
															<input type="text" class="form-control" value="<?=$Fechafact?>" required name="FechaPago" id="FechaPago"/>
														...
														<div class="col-sm-6">
															<div class="form-group">
                                                                ...
                                                                <div class="col-md-10">
															<input type="text" class="form-control" value="<?=$Contrato?>" onChange="javascript:this.value=this.value.toUpperCase();" required name="Contrato" id="Contrato"/>
													...
													-->	
															<div class="form-group">
																...
																<div class="col-lg-11 col-md-10 col-sm-9">
																	<br>
                                                                  <textarea name="Descripcion" rows="3" required class="form-control" id="Descripcion" placeholder="Descripcion" onChange="javascript:this.value=this.value.toUpperCase();"><?=$Descripcion?></textarea>
...
<div id="contenedor">
<?		  $query888="SELECT * FROM Movacta WHERE Idacta='$Id'";
    	  $result888=mysql_query($query888,$id);
    	  $cont=0;
    	  while ($row888=mysql_fetch_array($result888)) 
    	  {
    	  $Cant  		=$row888["Cant"];
    	  $Descri 		=$row888["Descripcion"];
    	  $Uni 			=$row888["Unidad"];
    	  $ValorUnidad 	=$row888["Neto"];
    	  ...
    	  ?>
    			<div class="added<?=$cont?>">
                                ...
                                <table width="100%" border="0" style="font-size:11px; border-color:#E4E4E4" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td width="10%"><input type="text" name="cant<?=$cont?>" id="cant<?=$cont?>" placeholder="Cantidad" style="width:99%; font-size:11px;" value="<?=$Cant?>" onKeyUp="puntitos(this,this.value.charAt(this.value.length-1));" required/></td>
                                    <td width="55%"><input type="text" name="descri<?=$cont?>" id="descri<?=$cont?>" placeholder="Descripcion" style="width:99%; font-size:11px;" value="<?=$Descri?>" onChange="javascript:this.value=this.value.toUpperCase();" onClick="window.open(&quot;lista-descripcion.php?Idcliente=<?=$Idcliente?>&Cont=<?=$cont?>&quot;,&quot;precios&quot;,&quot;menubar=no,scrollbars=yes, width=600, height=600&quot;);return false" required/></td>
                                    <td width="10%"><input type="text" name="unidad<?=$cont?>" id="unidad<?=$cont?>" placeholder="Unidad" style="width:99%; font-size:11px;" value="<?=$Uni?>" onChange="javascript:this.value=this.value.toUpperCase();" onChange="javascript:this.value=this.value.toUpperCase();" readonly/>
                                    <td width="10%"><input type="text" name="vlr<?=$cont?>" id="vlr<?=$cont?>" required placeholder="V/Unid" style="width:99%; font-size:11px;" value="<?=$ValorUnidad?>" onmouseout="puntitos(this,this.value.charAt(this.value.length-1));"/></td>
                                    <td width="10%" align="right">
    ...
    <?}?>
													...
													<div class="form-group">
														...
														?>
														<button type="button" id="visualizar" class="btn btn-success" onClick="window.open('print-actaobra.php?Id=<?=$Id?>');">Visualizar</button>
														<input name="Volver" type="button" class="btn btn-default" value="Volver"  onClick="location.href='act-actaobra.php?Idsubmenu=<?=$_REQUEST["Idsubmenu"]?>&Idmenu=<?=$_REQUEST["Idmenu"]?>&Idopcmenu=<?=$_REQUEST["Idopcmenu"]?>'">
														</div>
													</div>
                                                    </div>
