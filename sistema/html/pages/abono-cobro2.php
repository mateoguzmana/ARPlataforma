						...
						<div class="row">
							...
							$MosPerzB			=$rowPerz1["Del"];
								...
								$MosPerz2	=	1;
												...
												$queryDD			="SELECT * FROM HistorialCobros WHERE Idcuenta='$Idextra'";
												...
												$Cuota 				=$rowDD["Cuota"];
												$Retencion 			=$rowDD["Retencion"];
												
												$queryPRO			="SELECT * FROM Retenciones WHERE Valor='$Retencion' AND Muestra=0";
												$resultPRO 			=mysql_query($queryPRO,$id);

												while ($rowPRO=mysql_fetch_array($resultPRO)) {
												$ValRetencion 		=$rowPRO["Nombre"];
												}	

                                                $queryUSERS			="SELECT * FROM Facturacion WHERE Id='$Idextra' Order by Id DESC";
												...
												<tr class="gradeA">
													...
													<td class="text-right">
														...
														{
														?>                                              
													<input name="Procesar" type="button" class="btn btn-primary" value="VER" style="width:70px;"  onClick="window.open('print-abonocobro.php?Id=<?=$Idextra?>&Idpago=<?=$Idpago?>&Idcliente=<?=$IdclienteUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>','_blank')"> 
                                                        <?
														}
														elseif($Estado == 1)
														{
														?>
													<input name="Procesar" type="button" class="btn btn-warning" value="VER"  style="width:100px;" onClick="window.open('print-abonocobro.php?Id=<?=$Idextra?>&Idpago=<?=$Idpago?>&Idcliente=<?=$IdclienteUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>','_blank')">
                                                        <?
														}
														elseif($Estado == 2)
														{
