						...
						<div class="row">
							...
							$MosPerzB			=$rowPerz1["Del"];
								...
								$MosPerz2	=	1;
                                                ...
                                                $queryUSERS			="SELECT * FROM Controlbox Order by Id";
                                                $resultUSERS		=mysql_query($queryUSERS, $id);
                                                
                                                While($rowUSERS		=mysql_fetch_array($resultUSERS))
                                                {
                                                $NombreUSERS		=$rowUSERS["Guia"];
                                                $CedulaUSERS		=$rowUSERS["Remitente"];
												$ContactoUSERS		=$rowUSERS["Destinatario"];
												$CiudadUSERS		=$rowUSERS["Empresa"];
												$IdUSERS			=$rowUSERS["Id"];
												
											 	$queryHH 			="SELECT * FROM Clientes WHERE Id='$CiudadUSERS'";
											 	$resultHH 			=mysql_query($queryHH,$id);
											 	while ($rowHH		=mysql_fetch_array($resultHH)) {
											 	$NombreEmpresa 		=$rowHH["Nombre"];
											 	}
                                                ?>
												...
												<tr class="gradeA">
													...
													<td class="text-right">
														...
														{
														?>                                                    
														<input name="Procesar" type="button" class="btn btn-primary" value="MODIFICAR"  onClick="location.href='act-guias-2.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'"> 
                                                        <?
														}
														?> 
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <button type="button" class="btn btn-success" onclick="location.href='eli-guias.php?Id=<?=$IdUSERS?>&Idmenu=<?=$Idmenux?>&Idsubmenu=<?=$Idsubmenux?>&Idopcmenu=<?=$Idopcmenux?>'">Confirmar</button>
                                                            </div>
                                                        </div><!-- /.modal-content -->
                                                    </div><!-- /.modal-dialog -->
                                                </div><!-- /.modal -->
