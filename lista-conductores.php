...
{ 
   window.opener.document.form1.Conductor.value=cedula; 
   window.opener.document.form1.NOMBREC.value=nombre; 
   window.opener.document.form1.APELLIDOC.value=apellido; 
   window.opener.document.form1.CIUDADC.value=ciudad; 
   window.opener.document.form1.TELC1.value=tel1;
   window.opener.document.form1.TELC2.value=tel2;
   window.opener.document.form1.CELC.value=cell;
   window.close(); 
} 
</script>
...
<body>
<form name="form1" method="get" action="lista-conductores.php">
  <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td height="100" align="right" valign="top"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          <td width="30%" height="35" bgcolor="#003366" class="listado2">&nbsp;<span class="listado1">&nbsp; </span>NOMBRE CONDUCTOR<br>
            <span class="listado3"> &nbsp;
              <input name="Marca" type="text" class="TextField" id="Marca" style="width:95%">
            </span></td>
          <td width="30%" bgcolor="#003366" class="listado2">&nbsp;<span class="listado1">&nbsp; </span>CEDULA CONDUCTOR<br>
            <span class="listado3"> &nbsp;
	...
	elseif ($Cedula1 <> '')
	{
	$paging->agregarConsulta("SELECT* FROM Conductores where Cedula = '$Cedula1' order by Apellido1 Asc"); 
	}
	else
	{
...
?>
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" onClick="retornavalores('<?=$Cedula ?>', '<?=$Nombrer ?>', '<?=$Apellido?>', '<?=$Nombreciu?>', '<?=$Tel1?>', '<?=$Tel2?>', '<?=$Celular?>')">
          <tr name="datos1" id="datos1" onMouseover="this.bgColor='#BED9F3'"onMouseout="this.bgColor='#FBFBFB'" >
            <td width="15%" height="20"><span class="Estilo93">
              <?=$Cedula?>
            </span></td>
