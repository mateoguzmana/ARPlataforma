...
<script> 
function retornavalores(registro,ced)
{ 
   
   window.opener.document.form1.Captador.value=registro; 
   window.opener.document.form1.Cedcap.value=ced; 
   window.close(); 
} 
</script>
...
<body>
<form name="forma" method="get" action="lista-usuarioc.php">
  <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td height="100" align="right" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td width="45%" height="35" bgcolor="#003366" class="listado2">&nbsp;<span class="listado1">&nbsp; </span>NOMBRE USUARIO<br>
              <span class="listado3">
              &nbsp;
              <input name="Marca" type="text" class="TextField" id="Marca" style="width:98%">
            </span></td>
	...
	if ($Marca1 <> '')
	{
	$paging->agregarConsulta("SELECT* FROM Usuarios where Nombrefull Like '%$Marca1%' and  Tipo <> '2' order by Nombrefull Asc"); 
	}
	else
	{
	$paging->agregarConsulta("SELECT* FROM Usuarios where  Tipo <> '2' order by Nombrefull Asc"); 
	}
...
{	
$Nombre			=utf8_encode($row008["Nombrefull"]);
$Cedula			=$row008["Cedula"];
?>
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" onClick="retornavalores('<?=$Nombre?>', '<?=$Cedula?>')">
        
            <tr name="datos1" id="datos1" onMouseover="this.bgColor='#BED9F3'"onMouseout="this.bgColor='#FBFBFB'" >
              <td width="80%" height="20"><span class="Estilo93"><?=strtoupper($Nombre)?></span></td>
